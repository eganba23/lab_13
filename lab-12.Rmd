...
# Filter for Extraversion and Openness
extraversion_data <- colonies_stats %>% filter(big_5 == "Extraversion")
openness_data <- colonies_stats %>% filter(big_5 == "Openess")

# Plot 1: Histogram of mean Extraversion
ggplot(extraversion_data, aes(x = Mean)) +
  geom_histogram(color = "black", fill = "blue", bins = 20) +
  labs(
    x = "Mean Extraversion", 
    y = NULL,
...
# Plot 2: Histogram of Extraversion Standard Deviation
ggplot(extraversion_data, aes(x = `Standard Deviation`)) +
  geom_histogram(color = "black", fill = "red3", bins = 20) +
  labs(
    x = "SD Extraversion",
    y = NULL,
...
# Plot 3: Correlation of Extraversion and Openness means across colonies
# First, join extraversion and openness data
cor_data <- extraversion_data %>%
  select(colony, mean_ex = Mean) %>%
  left_join(openness_data %>% select(colony, mean_op = Mean), by = "colony")

ggplot(cor_data, aes(x = mean_ex, y = mean_op)) +
  geom_point(alpha = .6) +
  geom_smooth(method = 'lm', se = FALSE, formula = y ~ x) +
  ggpubr::stat_cor(method = "pearson", label.x = -.35, label.y = .03) +
  labs(
    x = "Extraversion Means",
    y = "Openness Means",
    title = "Correlation of Extraversion and Openness across Colonies"
